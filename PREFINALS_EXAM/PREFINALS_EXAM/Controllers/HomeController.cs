    ...
    public class HomeController : Controller
    {
        ...
        private List<ExamItem> GetExamQuestions()
        {
            return new List<ExamItem>
            {
                ...
                new ExamItem { Id = 13, Category = "Data Normalization & Data Validation", Question = "13. Which type of validation occurs in the browser before a request is sent to the server?", Options = new List<string>{ "A. Database-level validation", "B. Client-side validation", "C. Server-side validation", "D. EF Core migration validation" }, CorrectAnswer = 'B', AnswerText = "B. Client-side validation" },
                new ExamItem { Id = 14, Category = "Data Normalization & Data Validation", Question = "14. Why is server-side validation still necessary if client-side validation exists?", Options = new List<string>{ "A. Client-side validation can be bypassed", "B. Client-side validation automatically modifies the database", "C. Server-side validation only works with SQLite", "D. Client-side validation cannot display messages" }, CorrectAnswer = 'A', AnswerText = "A. Client-side validation can be bypassed" },
                new ExamItem { Id = 15, Category = "Data Normalization & Data Validation", Question = "15. A school requires every student to have a unique Student Number. Which rule best represents this requirement?", Options = new List<string>{ "A. Student Number should always be nullable", "B. Student Number should be unique", "C. Student Number should always be the same", "D. Student Number should contain only spaces" }, CorrectAnswer = 'B', AnswerText = "B. Student Number should be unique" },
                new ExamItem { Id = 16, Category = "SQL & In-Memory CRUD Operations", Question = "16. Which is the best reason for having a database-level unique constraint on StudentNumber?", Options = new List<string>{ "A. It protects data integrity even if application-level validation is bypassed", "B. It makes Razor Views render faster", "C. It removes the need for a Controller", "D. It automatically creates a ViewModel" }, CorrectAnswer = 'A', AnswerText = "A. It protects data integrity even if application-level validation is bypassed" },
